...
const comprarBtn = document.getElementById('finalizar-compra');
...
const ordenarAvaliacao = document.getElementById('ordenar-avaliacao');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ordenarAvaliacao.addEventListener('change', aplicarFiltros);
...
function criarProduto(produto) {
    ...
    const menosInfo = document.getElementById('menos-info');

menosInfo.addEventListener('click', () => {
...
const eliminarProdutos = document.getElementById('remover-produtos');

eliminarProdutos.addEventListener('click', () => {
...
function aplicarFiltros() {
    ...
    if (termoBusca) {
        produtosFiltrados = produtosFiltrados.filter(produto => produto.title.toLowerCase() && produto.description.toLowerCase().includes(termoBusca));
    }

    // Ordenar por avaliação
    const ordem = ordenarAvaliacao.value;
    if (ordem === 'crescente') {
...
function comprar() {
    ...
    const isEstudante = document.getElementById('estudante-deisi').checked;

    // Obtém o cupom de desconto
    const cupom = document.getElementById('cupom-desconto').value.trim();
